                            ...
                            return (
                                <tr key={item.id} className={`group transition-colors hover:bg-slate-50/50 ${!isValid ? 'bg-red-50/50' : ''}`}>
                                    ...
                                    <td className="py-3 px-4 text-center">
                                        <button onClick={() => onDeleteItem(item.id)} className="w-10 h-10 flex items-center justify-center text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors mx-auto active:scale-95">×</button>
                                    </td>
                                </tr>
                            );
                        })}
                    ...
                    return (
                        <div key={item.id} className={`p-3 space-y-2 ${!isValid ? 'bg-red-50/50 border-l-4 border-l-red-400 pl-2' : ''} text-sm transition-colors`}>
                            ...
                            <div className="flex items-center gap-2 h-8">
                                ...
                                <button
                                    onClick={() => handleModeSwitch(item.id, item.splitMode)}
                                    className={`shrink-0 h-10 px-3 rounded-lg border uppercase tracking-wider text-[10px] font-bold transition-all active:scale-95 ${item.splitMode === 'EQUAL' ? 'bg-sky-50 text-sky-600 border-sky-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}
                                >
                                ...
                                {/* Delete */}
                                <button onClick={() => onDeleteItem(item.id)} className="shrink-0 w-10 h-10 flex items-center justify-center text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors active:scale-95">
